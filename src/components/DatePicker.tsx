...
import { DatepickerCtx, useDatepickerCtx } from "../config/DatepickerContext";
  ...
  return (
    <DatepickerCtx.Provider value={ctxValue}>
      <Manager>
        <Reference>
          {({ ref }) => (
            <div className="flex p-0 m-0" ref={ref}>
              <input
                className="input--primary border-opacity-50"
                type="text"
                style={inputStyle}
...
const YearSelection: React.FC<{}> = (props) => {
  ...
  for (let i = minYear; i < maxYear; i++) {
    years.push(
      <CalButton key={i} onClick={(e) => selectYear(i)}>
        {i}
      </CalButton>
    );
  }
